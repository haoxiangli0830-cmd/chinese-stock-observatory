...
export async function GET() {
  ...
  const watchlistRows = stocks
    .map(
      (stock) =>
        `<tr><td>${stock.active ? "启用" : "停用"}</td><td>${stock.symbol}</td><td>${escapeHtml(stock.nameZh)}</td><td>${stock.exchange}</td><td>${escapeHtml(stock.category)}</td><td>${escapeHtml(stock.source)}</td><td>${escapeHtml(stock.lastSuccessAt ?? "待更新")}</td></tr>`,
    )
    .join("");
  ...
  <h2>观察名单</h2>
  <table><tr><th>状态</th><th>代码</th><th>名称</th><th>交易所</th><th>分类</th><th>数据源</th><th>最后更新</th></tr>${watchlistRows}</table>
